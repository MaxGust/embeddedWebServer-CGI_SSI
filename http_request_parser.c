...
int parseRquest_identifyRequest(char* requestBuffer,http_request_t* httpRequest)
{
  ...
  if(strstr(requestBuffer,"?")){ //if there is a query string, path ends at the start of query
    endOfPath  = strchr(startOfPath, '?');
  }
  else if(strstr(requestBuffer,"HTTP/1.")){  //if there is a HTTP version mentioned, path ends there
    endOfPath  = strchr(startOfPath, ' ');
  }
  else{
